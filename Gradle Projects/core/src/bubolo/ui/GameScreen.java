 ...
 */
public class GameScreen extends Screen implements NetworkObserver
{
	 ...
	 */
	public GameScreen(GameApplication app, World world)
	{
		this.app = app;
		this.world = world;
		...
		createButtonRow(skin);
		

	}
	private void createButtonRow(Skin skin)
	{
		...
		forwardButton.addListener(new ClickListener() {
			...
			{
				//Gdx.app.log("debug", "button clicked *** " + tank.getSpeed() );
				//tank.accelerate();
			}
		});
	    ...
	    table.add(forwardButton).expandX().width(50.f);
	    table.add(leftButton).expandX().width(50.f);
	}
